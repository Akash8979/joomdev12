...
session_start();
// session_unset();
if (isset($_GET["token"]) && !isset($_SESSION["auth"])) {
    $token = $_GET["token"];
    ...
    $select_token = "select * from users where remember_token = '$decoded_token'";
    $query = mysqli_query($conn, $select_token);
    if (mysqli_num_rows($query) > 0) {

        $_SESSION["auth"] = true;
        $user = mysqli_fetch_assoc($query);
        $destroy_token = "update users set remember_token = '0' where id = '{$user['id']}'";
        mysqli_query($conn, $destroy_token);
        header("location:index.php");
    } else {
        echo "token mismatch";
        die();
    }
} else if (!isset($_GET["token"]) && !isset($_SESSION["auth"])){
    header("location:http://localhost/joomdev/public/api/login-check");
}else {
    // $_SESSION["auth"] = true;
    if (empty($_SESSION['auth'])) {
        echo "token mismatch13212";
        echo $_SESSION["auth"];
        die();
    }
}
    ...
    <div class="container-fullscreen">
        ...
        <div class="container-sidebar hidden" id="option-tabs">
            ...
            <div id="get-options" class="text-center">
                ...
                <div class="get-options choose" data-id="content" id="content"><span class="glyphicon glyphicon-list-alt"></span>
                    <div>Text</div>
                </div>
                <div class="get-options choose" data-id="image" id="image"><span class="glyphicon glyphicon-picture"></span>
                    <div>Image</div>
                </div>
                <div class="get-options choose" data-id="video" id="video"><span class="glyphicon glyphicon-play"></span>
                    <div>Video</div>
                </div>
                <div class="get-options choose" data-id="link" id="link"><span class="glyphicon glyphicon-link"></span>
                    <div>Link</div>
                </div>
                <div class="get-options choose" data-id="divider" id="divider"><span class="glyphicon glyphicon-minus"></span>
                    <div>Divider</div>
                </div>
